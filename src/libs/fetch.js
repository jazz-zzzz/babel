    ...
    GM.xmlHttpRequest({
      ...
      onload: ({ response, responseHeaders, status, statusText }) => {
        ...
        try {
          ...
          babelLog("fetchGM parse headers error", e);
        }
...
export const fetchPatcher = async (input, init = {}, opts) => {
  ...
  if (!timeout) {
    try {
      ...
      babelLog("getSettingWithDefault", err);
    }
  }
  if (!timeout) {
  ...
  if (isGm) {
    ...
    const { body, headers, status, statusText } = window.BABEL_GM
      ? await window.BABEL_GM.fetch(input, init)
...
async function* fetchStreamGM(
  ...
  const gmRequest = window.BABEL_GM?.xmlHttpRequest || GM.xmlHttpRequest;
