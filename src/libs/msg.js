...
const isIgnorableMessageError = (err) => {
...
export const sendBgMsg = (action, args) => {
  try {
    if (!browser?.runtime?.sendMessage) return Promise.resolve();
    return browser.runtime.sendMessage({ action, args }).catch((err) => {
      if (isIgnorableMessageError(err)) return undefined;
      throw err;
    });
  } catch (err) {
    if (isIgnorableMessageError(err)) return Promise.resolve();
...
export const sendTabMsg = async (action, args) => {
  ...
  return browser.tabs.sendMessage(tabId, { action, args }).catch((err) => {
    ...
    if (isIgnorableMessageError(err)) {
