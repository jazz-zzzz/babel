...
async function set(key, val) {
  if (isExt) {
    ...
    await (window.BABEL_GM || GM).setValue(key, val);
  } else {
...
async function get(key) {
  if (isExt) {
    ...
    const val = await (window.BABEL_GM || GM).getValue(key);
    return val;
  }
...
async function del(key) {
  if (isExt) {
    ...
    await (window.BABEL_GM || GM).deleteValue(key);
  } else {
...
async function trySetObj(key, obj) {
  if ((await getObjWithFallback(key)) === null) {
...
async function getObj(key) {
  ...
  try {
    ...
    babelLog("parse json in storage err: ", key);
  }
  return null;
}

async function getObjWithFallback(key) {
  const current = await getObj(key);
  if (current !== null) {
    return current;
  }

  const fallbackKeys = STORAGE_FALLBACK_KEYS[key] || [];
  for (const fallbackKey of fallbackKeys) {
    const fallback = await getObj(fallbackKey);
    if (fallback !== null) {
      await setObj(key, fallback);
      return fallback;
    }
  }
...
async function putObj(key, obj) {
  const cur = (await getObjWithFallback(key)) ?? {};
...
// --- 应用设置 (Settings) 数据存取 ---
export const getSetting = () => getObjWithFallback(STOKEY_SETTING);
export const getSettingOld = () => getObjWithFallback(STOKEY_SETTING_OLD);
export const getSettingWithDefault = async () => {
  ...
  return {
    ...setting,
    transApis: normalizeTransApisForRuntime(setting.transApis),
  };
};
...
// --- 用户翻译规则 (Rules) 数据存取 ---
export const getRules = () => getObjWithFallback(STOKEY_RULES);
export const getRulesOld = () => getObjWithFallback(STOKEY_RULES_OLD);
...
export const getWords = () => getObjWithFallback(STOKEY_WORDS);
...
export const getDisabledSubRules = async (url) => {
  if (!url) return [];
  const raw = await getObjWithFallback(STOKEY_DISABLED_SUB_RULES);
  if (!raw) return [];
...
export const setDisabledSubRules = async (url, patterns) => {
  if (!url) return;
  const map = (await getObjWithFallback(STOKEY_DISABLED_SUB_RULES)) || {};
...
export const getFab = () => getObjWithFallback(STOKEY_FAB);
...
export const getTranBox = () => getObjWithFallback(STOKEY_TRANBOX);
...
export const getSync = () => getObjWithFallback(STOKEY_SYNC);
...
// --- 微软云服务授权 Token 存取 ---
export const getMsauth = () => getObjWithFallback(STOKEY_MSAUTH);
export const setMsauth = (val) => setObj(STOKEY_MSAUTH, val);

// --- 百度云服务授权 Token 存取 ---
export const getBdauth = () => getObjWithFallback(STOKEY_BDAUTH);
