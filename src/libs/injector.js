...
export const injectInlineJs = (code, id = "babel-inline-js") => {
  // 避免重复注入相同 ID 的脚本
  ...
  el.setAttribute("data-source", "babel-inject injectInlineJs");
...
export const injectInlineJsBg = (code, id = "babel-inline-js") => {
  if (document.getElementById(id)) {
    return;
  }

  const el = document.createElement("script");
  el.setAttribute("data-source", "babel-inject injectInlineJsBg");
  el.type = "text/javascript";
  el.id = id;
  el.textContent = code;
...
export const injectExternalJs = (src, id = "babel-external-js") => {
  if (document.getElementById(id)) {
    return;
  }

  const el = document.createElement("script");
  el.setAttribute("data-source", "babel-inject injectExternalJs");
  el.type = "text/javascript";
  el.id = id;
...
export const injectInternalCss = (styles) => {
  ...
  el.setAttribute("data-source", "babel-inject injectInternalCss");
...
export const injectExternalCss = (href) => {
  ...
  el.setAttribute("data-source", "babel-inject injectExternalCss");
