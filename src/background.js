...
import { babelLog, logger } from "./libs/log";
import { chromeDetect, chromeTranslate } from "./libs/builtinAI";

globalThis.__BABEL_CONTEXT__ = "background";
...
async function persistSeparateWindowBounds(bounds) {
  ...
  try {
    ...
    babelLog("Final separate window bounds saved to storage", bounds);
  } catch (err) {
    babelLog("Save separate window bounds error", err);
  }
}
  ...
  try {
    ...
    if (separateWindowId !== null) {
      ...
      if (existingWin) {
        ...
        babelLog("Separate window is ready");
        return existingWin;
      }
    }
    ...
    babelLog("open separate window error", err);
  }
}
...
async function updateCacheFromActual(windowId) {
  try {
    ...
    if (win && win.state === "normal") {
      ...
      babelLog("Bounds cached via fallback:", lastKnownBounds);
 ...
 */
let contextMenusUpdatePromise = Promise.resolve();
    ...
    try {
      browser.contextMenus.create(props, () => {
        ...
        if (error && !error.message?.includes("duplicate id")) {
          babelLog("create contextMenu", error);
        }
        resolve();
      });
    } catch (err) {
      babelLog("create contextMenu", err);
      resolve();
    }
  });

async function addContextMenus(contextMenuType = 1) {
  contextMenusUpdatePromise = contextMenusUpdatePromise
    .catch(() => {})
    .then(() => rebuildContextMenus(contextMenuType));
  return contextMenusUpdatePromise;
}
...
async function rebuildContextMenus(contextMenuType = 1) {
  try {
    ...
    babelLog("remove contextMenus", err);
  }
...
async function getUiLanguage() {
  try {
    ...
    babelLog("get UI language error", err);
    return "en";
  }
}
...
browser.runtime.onInstalled.addListener(async () => {
  ...
  await addContextMenus(contextMenuType);
