...
export class YouTubeSubtitleList {
  ...
  constructor(videoElement) {
    ...
    document.addEventListener("babel-add-word", this.handleWordAdded);

    // 监听来自扩展配置选项页面等第三方发送的消息，用以点击生词时同步跳转视频进度
    window.addEventListener("message", (event) => {
      if (event.data && event.data.type === "BABEL_JUMP_TO_TIME") {
  ...
  destroy() {
    ...
    document.removeEventListener("babel-add-word", this.handleWordAdded);
    if (this.container) {
  ...
  downloadSubtitles() {
    ...
    try {
      ...
      downloadBlobFile(
        vttContent,
        `babel-subtitles-${videoId}_${Date.now()}.vtt`
      );
    } catch (error) {
  ...
  _ensureContainer() {
    this.container = document.getElementById(
      "babel-youtube-subtitle-list-container"
    );
    if (!this.container) {
      this.container = document.createElement("div");
      this.container.id = "babel-youtube-subtitle-list-container";
  ...
  _renderTabsAndStructure() {
    ...
    this.subtitleListEl.id = "babel-youtube-subtitle-list";
    ...
    this.vocabularyListEl.id = "babel-youtube-vocabulary-list";
  ...
  _createSubtitleListItem(sub, index) {
    ...
    li.id = `babel-youtube-item-${index}`;
    li.className = "babel-youtube-item";
  ...
  updateSingleSubtitle(subtitleUpdate) {
    ...
    if (item) {
      const translationEl = item.querySelector(".babel-youtube-translation");
      if (translationEl) {
  ...
  _downloadFile(content, mimeType, extension) {
    ...
    downloadBlobFile(
      blob,
      `babel-vocabulary-${new Date().toISOString().slice(0, 10)}.${extension}`
    );
  }
