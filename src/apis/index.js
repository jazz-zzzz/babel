...
import {
  getHttpCachePolyfill,
  putHttpCachePolyfill,
  tryClearCaches,
} from "../libs/cache";
...
/**
 * 批量翻译字幕片段。YouTube 字幕会按 chunk 调用此方法，并用相邻上下文辅助翻译。
 * @param {Object} params
 * @param {string[]} params.texts - 所有待翻译的字幕原文数组
 * @param {string} params.fromLang - 源语言
 * @param {string} params.toLang - 目标语言
 * @param {Object} params.apiSetting - 翻译接口配置
 * @param {Object} params.docInfo - 视频/文档上下文信息
 * @param {Array<{globalIndex:number,text:string}>} params.contextBefore - chunk 前文上下文
 * @param {Array<{globalIndex:number,text:string}>} params.contextAfter - chunk 后文上下文
 * @param {boolean} params.strict - 是否启用严格 chunk JSON 解析
 * @param {AbortSignal} params.signal - 取消信号
 * @returns {Promise<string[]>} 按 texts 索引对应的翻译结果数组
 */
export const apiBatchSubtitleTranslate = async ({
  texts,
  fromLang = "auto",
  toLang,
  apiSetting,
  ...
  const { apiSlug, apiType } = apiSetting;

  // 缓存键：videoId + apiSlug + 起止文本摘要 → 不同视频/配置不会串缓存
  const cacheInput = `${URL_CACHE_SUBTITLE}?${queryString.stringify({
    apiSlug,
    videoId: videoId || "",
    fromLang,
    toLang,
    count: texts.length,
    first: texts[0]?.slice(0, 50) || "",
    last: texts[texts.length - 1]?.slice(0, 50) || "",
    strict: strict ? 1 : 0,
    batchVer: 10,
  })}`;

  const cache = await getHttpCachePolyfill(cacheInput);
  if (isCompleteSubtitleTranslationBatch(texts, cache)) return cache;
  if (signal?.aborted) return [];

  // AI 接口：构建批量子幕专用的 systemPrompt（完全替换，非追加）
  //   详细提示词和配置统一由 createBatchSubtitleApiSetting 生成
  const isAI = API_SPE_TYPES.ai.has(apiType);
  const enhancedApiSetting = createBatchSubtitleApiSetting({
    apiSetting,
    isAI,
    texts,
    toLang,
  ...
  // 语言代码映射：fromLang/toLang → API 专用格式（如 "zh-CN" → "Chinese (Simplified)"）
  const langMap = OPT_LANGS_TO_SPEC[apiType] || OPT_LANGS_SPEC_DEFAULT;
  const mappedFrom = langMap.get(fromLang) || fromLang;
  const mappedTo = langMap.get(toLang) || toLang;

  for await (const item of handleTranslate(texts, {
    from: mappedFrom,
    to: mappedTo,
    fromLang,
    toLang,
    ...
    if (item.result && item.id < texts.length) {
      translations[item.id] = String(item.result[0] || "");
    }
  }

  if (isCompleteSubtitleTranslationBatch(texts, translations)) {
    putHttpCachePolyfill(cacheInput, null, translations);
  }
  return translations;
};

// 清理由 segVer: 2 分块旧缓存，迁移到 segVer: 3 全量缓存（仅执行一次）
const CACHE_MIGRATION_KEY = "babel_segver3_cache_migrated";
if (
  typeof localStorage !== "undefined" &&
  !localStorage.getItem(CACHE_MIGRATION_KEY)
) {
  tryClearCaches();
  try {
