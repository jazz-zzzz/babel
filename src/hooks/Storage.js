...
export function useStorage(key, defaultVal = null, syncKey = "") {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        const storedVal = await storage.getObjWithFallback(key);
        ...
        babelLog(`storage load error for key: ${key}`, err);
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  const runSync = useCallback(async (keyToSync, valueToSync) => {
    try {
      ...
      babelLog("Sync failed", keyToSync);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    storage.setObj(key, data).catch((err) => {
      babelLog(`storage save error for key: ${key}`, err);
    });
  ...
  const remove = useCallback(async () => {
    try {
      ...
      babelLog(`storage remove error for key: ${key}`, err);
    }
  }, [key]);
  ...
  const reload = useCallback(async () => {
    try {
      ...
      babelLog(`storage reload error for key: ${key}`, err);
    }
  }, [key, defaultVal]);
