...
import { MSG_OPEN_TRANBOX, EVENT_BABEL_INNER } from "../config";
...
export default function useTranboxShortcuts({
  ...
  useEffect(() => {
    ...
    document.addEventListener(EVENT_BABEL_INNER, handleStatusUpdate);
    return () => {
      document.removeEventListener(EVENT_BABEL_INNER, handleStatusUpdate);
    };
  }, [handleToggle]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      babelLog("registerMenuCommand", err);
    }
