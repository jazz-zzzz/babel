...
export const I18N = {
  ...
  sync_warn: {
    zh: `涉及隐私数据的同步请谨慎选择第三方同步服务，建议自行搭建 babel-worker 或 WebDAV 服务。`,
    en: `When synchronizing data that involves privacy, please be cautious about choosing third-party sync services. It is recommended to set up your own sync service using babel-worker or WebDAV.`,
    zh_TW: `同步涉及隱私資料時，請謹慎選擇第三方同步服務；建議自行架設 babel-worker 或 WebDAV 服務。`,
    ja: `プライバシーに関わるデータを同期する場合、サードパーティの同期サービスは慎重に選択してください。babel-worker や WebDAV サービスを自己ホスティングすることをお勧めします。`,
    ko: `개인정보가 포함된 데이터를 동기화할 경우, 타사 동기화 서비스 선택에 신중을 기하십시오. 자체 babel-worker 또는 WebDAV 서비스를 구축하는 것을 권장합니다.`,
  },
  sync_warn_2: {
  ...
  about_sync_api: {
    zh: `自建babel-worker数据同步服务`,
    en: `Self-hosting a Babel-worker data sync service`,
    zh_TW: `自行架設 babel-worker 資料同步服務`,
    ja: `Babel-worker データ同期サービスをセルフホストする`,
    ko: `Babel-worker 데이터 동기화 서비스 자체 호스팅`,
  },
  about_api: {
  ...
  fixer_function_helper: {
    zh: `1、br是将<br>换行替换成<p "babel-p">。2、bn是将\\n换行替换成<p "babel-p">。3、brToDiv和bnToDiv是替换成<div class="babel-p">。`,
    en: `1. br replaces <br> line breaks with <p "babel-p">. 2. bn replaces \\n newline with <p "babel-p">. 3. brToDiv and bnToDiv are replaced with <div class="babel-p">.`,
    zh_TW: `1. br 會將 <br> 換行替換為 <p "babel-p">。2. bn 會將 \\n 換行替換為 <p "babel-p">。3. brToDiv 與 bnToDiv 會替換為 <div class="babel-p">。`,
    ja: `1. br は <br> 改行を <p "babel-p"> に置き換えます。 2. bn は \\n 改行を <p "babel-p"> に置き換えます。 3. brToDiv と bnToDiv は <div class="babel-p"> に置き換えます。`,
    ko: `1. br은 <br> 줄바꿈을 <p "babel-p">로 대체합니다. 2. bn은 \\n 줄바꿈을 <p "babel-p">로 대체합니다. 3. brToDiv 및 bnToDiv는 <div class="babel-p">로 대체됩니다.`,
  },
  import: {
    zh: `导入`,
    en: `Import`,
  ...
  error_sync_setting: {
    zh: `您的同步类型必须为“Babel-Worker”，且需填写完整`,
    en: `Your sync type must be "Babel-Worker" and must be filled in completely`,
    zh_TW: `您的同步型態必須為「Babel-Worker」，且需填寫完整。`,
    ja: `同期タイプは「Babel-Worker」である必要があり、すべて入力する必要があります。`,
    ko: `동기화 유형은 "Babel-Worker"여야 하며, 모든 항목을 빠짐없이 입력해야 합니다.`,
  },
  click_test: {
    zh: `点击测试`,
    en: `Click Test`,
  ...
  context_doc_info: {
    zh: `页面元数据上下文`,
    en: `Page metadata context`,
    zh_TW: `頁面元資料上下文`,
    ja: `ページメタデータコンテキスト`,
    ko: `페이지 메타데이터 컨텍스트`,
  },
  context_subtitle_adjacent: {
    zh: `字幕前后句上下文`,
    en: `Subtitle adjacent context`,
    zh_TW: `字幕前後句上下文`,
    ja: `字幕の前後文脈`,
    ko: `자막 인접 컨텍스트`,
  },
  context_chat_history: {
    zh: `多轮对话历史`,
    en: `Chat history context`,
    zh_TW: `多輪對話歷史`,
    ja: `会話履歴コンテキスト`,
    ko: `대화 기록 컨텍스트`,
  },
  context_video_summary: {
    zh: `AI 视频摘要`,
    en: `AI video summary`,
    zh_TW: `AI 影片摘要`,
    ja: `AI動画要約`,
  ...
  context_chat_history_hint: {
    zh: `将前几句翻译作为上下文发送，帮助 AI 保持术语一致`,
    en: `Send previous translations as context to maintain term consistency`,
    zh_TW: `將前幾句翻譯作為上下文發送，幫助 AI 保持術語一致`,
    ja: `前の翻訳をコンテキストとして送信し、用語の一貫性を維持`,
    ko: `이전 번역을 컨텍스트로 전송하여 용어 일관성 유지`,
  },
  context_video_summary_hint: {
    zh: `额外调用 AI 分析视频内容后再翻译，提升准确度但增加 API 消耗`,
    en: `Analyze video content via AI before translating; improves accuracy at extra API cost`,
    zh_TW: `額外呼叫 AI 分析影片內容後再翻譯，提升準確度但增加 API 消耗`,
    ja: `翻訳前にAIで動画内容を分析。精度向上するがAPI消費が増加`,
    ko: `번역 전 AI로 비디오 내용 분석. 정확도 향상되나 API 비용 증가`,
  },
  use_context: {
    zh: `是否启用智能上下文`,
  ...
  pause_on_subtitle_hover: {
    zh: `悬停字幕时暂停视频`,
    en: `Pause video on subtitle hover`,
    zh_TW: `懸停字幕時暫停影片`,
    ja: `字幕ホバー時に動画を一時停止`,
    ko: `자막에 마우스를 올리면 동영상 일시정지`,
  },
