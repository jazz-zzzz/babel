...
This is the COMPLETE subtitle transcript. Process it in one pass — do not truncate, skip, or summarize any portion.
...
4. Every input word MUST appear in exactly one output segment. Verify complete coverage before responding.
5. Non-speech cues like "[Music]" or "[Applause]" should appear as standalone segments with or without translation as context dictates.
...
const defaultApi = {
  ...
  contextDocInfo: true, // 页面元数据上下文（标题/描述）注入 Prompt
  contextChatHistory: false, // 多轮对话历史上下文
  contextSize: DEFAULT_CONTEXT_SIZE, // 对话历史保留会话数
...
const isUnsetOrEqual = (value, legacyValue) =>
  value === undefined || value === legacyValue;

const isUnsetOrNumberEqual = (value, legacyValue) =>
  value === undefined || Number(value) === legacyValue;

const usesLegacyDeepSeekSpeedDefaults = (api) =>
  api?.apiType === OPT_TRANS_DEEPSEEK &&
  isUnsetOrEqual(api.useStream, false) &&
  isUnsetOrEqual(api.streamRenderMode, "disabled") &&
  isUnsetOrEqual(api.thinkingMode, "auto") &&
  isUnsetOrNumberEqual(api.batchInterval, DEFAULT_BATCH_INTERVAL) &&
  isUnsetOrNumberEqual(api.fetchInterval, DEFAULT_FETCH_INTERVAL) &&
  isUnsetOrNumberEqual(api.rootMargin, 500);
