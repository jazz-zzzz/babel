...
export default function SyncSetting() {
  ...
  const handleGenerateShareString = async () => {
    try {
      ...
      babelLog("Share string copied to clipboard", shareString);
    } catch (error) {
      babelLog("Failed to copy share string to clipboard", error);
    }
  };
    ...
    try {
      ...
      if (text.startsWith(OPT_SYNCTOKEN_PERFIX)) {
        ...
        if (!OPT_SYNCTYPE_ALL.includes(updatedConfig.syncType)) {
          babelLog("error syncType", updatedConfig.syncType);
          return;
        }
        ...
        if (updatedConfig.syncUrl) {
          ...
          babelLog("Invalid config structure");
        }
      } else {
        babelLog("Invalid share string", text);
      }
    } catch (error) {
      babelLog("Failed to read from clipboard or parse JSON", error);
    }
  };
    ...
    <Box>
      <Stack spacing={3}>
        ...
        <TextField
          ...
          helperText={
            syncType === OPT_SYNCTYPE_WORKER && (
              <Link href={URL_BABEL_WORKER} target="_blank">
                {i18n("about_sync_api")}
              </Link>
            )
          }
